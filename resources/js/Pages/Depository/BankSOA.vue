...
const handleSaveSOA = async () => {
  if (!form.value.SOA)
    return Swal.fire('Warning', 'Please attach a PDF file first.', 'warning');
  ...
  try {
    ...
    Swal.fire('Success', 'SOA and transactions saved successfully!', 'success');
    await fetchSOA();

  } catch (error) {

    console.error(error);
    Swal.fire('Error', 'An issue occurred while saving SOA or transactions.', 'error');
    ...
    <div v-else-if="account" class="space-y-4">
      <div class="flex items-center justify-between border-b pb-2">
        <h2 class="text-lg font-semibold text-gray-800">
          Bank SOA — <span class="text-blue-600">{{ account.AccountNo }}</span>
        </h2>
      </div>
