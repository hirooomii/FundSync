...
import { parsePDF, parseAgriStatement } from '@/Utils/soaAGRI'
...
const handleSaveAGRIBANK = async () => {
  ...
  try {
    ...
    const pdfData = await parseAgriStatement(parsedBDOFile)
  ...
  const banktransactions = pdfData.transactions.map((t, i) => {
    ...
    const nextBalance = i + 1 < pdfData.transactions.length ? parseNum(pdfData.transactions[i + 1].running_balance) : null;
    ...
    if (nextBalance !== null) {
      ...
      if (t.description.includes('Withdrawal') || t.description.includes('Interest Tax')) {
        debit_or_credit = 'D';
