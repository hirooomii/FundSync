...
import { parsePDF, parseAUBStatement } from '@/Utils/soaAUB'
...
const handleSaveAUB = async () => {
  ...
  try {
    ...
    const pdfData = await parseAUBStatement(parsedFile)
    
    const excludeCheckNos = [
    'DATE',
    'BEGINNING BALANCE',
    'ENDING BALANCE',
    'Interest',
    ...
    const banktransactions = pdfData.transactions
    .filter((t) => {
        const checkNo = t.check_no?.trim();
        return checkNo && !excludeCheckNos.includes(checkNo);
    })
    .map((t) => {
        const checkNo = t.check_no?.trim();
        const isCredit = /credit/i.test(t.transaction_description);
        ...
        runningbal: parseFloat(t.credit) || 0,
        debit_or_credit: isCredit ? 'C' : 'D',
        transaction_type: isCredit ? 'Credit' : 'Debit',
        description: t.transaction_description,
        reference: checkNo,
        additional_info: t.transaction_description,
        };
    });
