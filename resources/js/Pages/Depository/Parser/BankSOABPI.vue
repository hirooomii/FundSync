...
import { parsePDF, parseBPIStatement } from '@/Utils/soaBPI'
...
const handleSaveBPI = async () => {
  ...
  loading.value = true
    ...
    try {
      const parsedBPIFile = await parsePDF(form.value.SOA)
      const pdfData = await parseBPIStatement(parsedBPIFile)

      const banktransactions = pdfData.transactions.map((t, i) => {
      const amount = (typeof t.amount === 'string') 
          ? parseFloat(t.amount.replace(/,/g, '')) || 0 
          : (typeof t.amount === 'number' ? t.amount : 0);

      const balance = (typeof t.running_balance === 'string') 
          ? parseFloat(t.running_balance.replace(/,/g, '')) || 0 
          : (typeof t.running_balance === 'number' ? t.running_balance : 0);

      const cleaned = t.description ? t.description.trim().split(/\s+/) : [];

      const next_balance = (i + 1 < pdfData.transactions.length)
          ? (typeof pdfData.transactions[i + 1].running_balance === 'string'
              ? parseFloat(pdfData.transactions[i + 1].running_balance.replace(/,/g, '')) || 0
              : (typeof pdfData.transactions[i + 1].running_balance === 'number'
                  ? pdfData.transactions[i + 1].running_balance
                  : 0))
          : null;
    ...
    <div v-else-if="account" class="space-y-4">
      <div class="flex items-center justify-between border-b pb-2">
        <h2 class="text-lg font-semibold text-gray-800">
          Bank SOA (BPI) — <span class="text-blue-600">{{ account.AccountNo }}</span>
        </h2>
      </div>
