...
import { parsePDF, parseBPOStatement } from '@/Utils/soaBDO'
...
const handleSaveBDO = async () => {
  ...
  loading.value = true
  try {
    const parsedBDOFile = await parsePDF(form.value.SOA)
    const pdfData = await parseBPOStatement(parsedBDOFile)

    const banktransactions = pdfData.transactions.map((t, i) => {
      const parseNum = (n) => (typeof n === 'string' ? parseFloat(n.replace(/,/g, '')) || 0 : n || 0)
      const amount = parseNum(t.amount)
      const balance = parseNum(t.running_balance)
      const prevBalance = i > 0 ? parseNum(pdfData.transactions[i - 1].running_balance) : null

      const debit_or_credit = balance > prevBalance ? 'C' : balance < prevBalance ? 'D' : 'N'
      const transaction_type =
        debit_or_credit === 'C' ? 'Credit' : debit_or_credit === 'D' ? 'Debit' : 'None'

      const cleaned = (t.bundle_name || '')
        .split(',')
        .map((p) => p.trim())
        .filter((p) => p && !/^\d{1,3}(,\d{3})*(\.\d+)?$/.test(p))
    ...
    <div v-else-if="account" class="space-y-4">
      <div class="flex items-center justify-between border-b pb-2">
        <h2 class="text-lg font-semibold text-gray-800">
          Bank SOA BDO — <span class="text-blue-600">{{ account.AccountNo }}</span>
        </h2>
      </div>
