...
import { parsePDF, parsePBCOMStatement } from '@/Utils/soaPBCOM'
...
const handleSavePBCOM = async () => {
  ...
  try {
    ...
    const pdfData = await parsePBCOMStatement(parsedBDOFile)
  ...
  const banktransactions = pdfData.transactions.map((t, i) => {
    ...
    const prev_balance =
        i > 0 ? parseNum(pdfData.transactions[i - 1].running_balance) : null;
    ...
    if (i === pdfData.transactions.length - 1 && debit_or_credit === 'N') {
        if (
        t.description.includes('Loan Charges') ||
        t.description.includes('Outward') ||
        t.description.includes('Tax Amount Due')
        ) {
        debit_or_credit = 'D';
