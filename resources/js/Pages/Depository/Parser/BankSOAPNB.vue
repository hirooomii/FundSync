...
import { parsePDF, parsePNBStatement } from '@/Utils/soaPNB'
...
const handleSavePNB = async () => {
  ...
  try {
    ...
    const pdfData = await parsePNBStatement(parsedFile)

    const banktransactions = pdfData.transactions.map((t) => {
    const withdrawal = parseFloat(t.withdrawal.replace(/,/g, '')) || 0;
    const deposit = parseFloat(t.deposit.replace(/,/g, '')) || 0;
    const balance = parseFloat(t.balance.replace(/,/g, '')) || 0;
    ...
    return {
        ...
        runningbal: balance,
        debit_or_credit: isWithdrawal ? 'C' : 'D',
        transaction_type: isWithdrawal ? 'Credit' : 'Debit',
        description: t.description,
        reference: t.checkSeq,
        additional_info: t.brCode ? t.brCode : '0000',
    };
    });
