...
import { parsePDF, parseUBStatement } from '@/Utils/soaUB'
...
const handleSaveUB = async () => {
  ...
  try {
    ...
    const pdfData = await parseUBStatement(parsedBDOFile)
  ...
  const banktransactions = pdfData.transactions.map((t) => {
    ...
    const running_balance = parseValue(t.running_balance);
    const debit = parseValue(t.debit);
    const credit = parseValue(t.credit);

    const amount = credit || debit;
    const debit_or_credit = credit ? 'C' : 'D';
    ...
    const cleanedDescription = (t.description || '').trim();
        ...
        return {
            ...
            bank_code: 'UNIONBANK',
            transaction_date: (t.date || '').split(',')[0].trim(),
            amount,
            runningbal: running_balance,
            debit_or_credit,
            transaction_type,
            description: cleanedDescription,
            reference: t.transaction,
            additional_info: t.check_reference,
        };
    });
