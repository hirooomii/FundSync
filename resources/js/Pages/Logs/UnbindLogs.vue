...
const fetchLogs = async () => {
  loading.value = true;
  try {
    const res = await axios.post('/get-unbind-logs', {
      draw: draw.value++,
      start: 0,
      length: 50,
...
const filteredLogs = computed(() => {
  ...
  return logs.value.filter(
    (l) =>
      ...
      String(l.acumatica_reference ?? '').toLowerCase().includes(q) ||
      String(l.bank_transaction_id ?? '').toLowerCase().includes(q) ||
  ...
  <AuthenticatedLayout>
    <template #header>
      <div class="flex items-center gap-2.5">
        ...
        <div>
          <h1 class="text-[15px] font-bold text-gray-800 leading-tight">Unbind Logs</h1>
          <p class="text-xs text-gray-400">Audit trail of unbound reconciliations</p>
        </div>
      </div>
    </template>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
              <thead class="bg-gray-50 text-xs font-medium text-gray-500 uppercase">
                <tr>
                  ...
                  <th class="px-4 py-3 text-left">Acumatica Reference</th>
                  <th class="px-4 py-3 text-left">Bank Transaction ID</th>
                  <th class="px-4 py-3 text-left">User</th>
                  <th class="px-4 py-3 text-left">Unbind Date</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200 bg-white">
                ...
                >
                  ...
                  <td class="px-4 py-3 text-gray-700">{{ log.acumatica_reference }}</td>
                  <td class="px-4 py-3 text-gray-700">{{ log.bank_transaction_id }}</td>
                  <td class="px-4 py-3 text-gray-600">{{ log.user }}</td>
                  <td class="px-4 py-3 text-gray-500">{{ log.unbind_date }}</td>
                </tr>
              </tbody>
            </table>
          </div>
